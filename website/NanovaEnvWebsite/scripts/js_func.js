$(function() {
	var readMoreFlag = 0;//if is 0, don't disply guideline, if is 1, disply guideline.
	var myName = $('#name');
	...
	$('.read_more btn_col').on('click', function(){
			if(readMoreFlag == 0)
			{
				$('.guidelinesDiv').display = block;
				readMoreFlag = 1;
			}
			else
			{
				$('.guidelinesDiv').display = none;
				readMoreFlag = 0;
			}
		});
});
  ...
  // media query change
    ...
    if (mq.matches) {
      document.getElementById("checkTable").className = ""
    } else {
      document.getElementById('checkTable').className = 'pricing_option pricing_option3';
    }
  }
