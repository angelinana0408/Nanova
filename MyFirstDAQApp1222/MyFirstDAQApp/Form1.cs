...
namespace MyFirstDAQApp
{
    public partial class Form1 : Form
    {
        ...
        public List<float> x2 = new List<float>();     //Set temp
        public List<float> y2 = new List<float>();
        public List<float> x3 = new List<float>();     //real temp
        ...
        private double analogIn1()   //1D signal
        {
        ...
        private double analogIn2()   //Temperature
        {
            Task analogInTask = new Task();
            AIChannel myAIChannel;
            myAIChannel = analogInTask.AIChannels.CreateVoltageChannel("dev1/ai3", "myAIChannel",
                AITerminalConfiguration.Differential, 0, 5, AIVoltageUnits.Volts);
            AnalogSingleChannelReader reader = new AnalogSingleChannelReader(analogInTask.Stream);
            double analogDataIn = reader.ReadSingleSample();
            return analogDataIn;
        ...
        private double ConvertSignToTemp(double Signalvalue)
        {
            double Tempvalue = 0;
            double EIndex = Math.Log(Signalvalue) * 0.51;
            Tempvalue = ((Math.Exp(EIndex) * 10 + 26.2) * 10.845 - 283.85)/1000;
            file.WriteLine(Signalvalue.ToString() + " " + Tempvalue.ToString());
            return Tempvalue;
        }
        ...
        private void timerDrawTemp_Tick(object sender, EventArgs e)//start timer and draw
        {
            ...
            double actualTemp = ConvertSignToTemp(analogIn2() * 175000 / 7 + 35000);
            x2.Add(DrawTempIndex);
            y2.Add((float)setTemp);
            x3.Add(DrawTempIndex);
            y3.Add((float)actualTemp);
            PIDControl(setTemp, actualTemp);
            DrawTempIndex = (float)(DrawTempIndex + 0.25);//for each count the x add 0.1
            TempGraph.f_Refresh();
        }

        /*******************************************************************************************************************/
        private void EndBeforeStartError()
        {
            timerMenu.Stop();
            timerDrawSignal.Stop();
            timerDrawTemp.Stop();
            MessageBox.Show("End time must be later than start time!", "Heartcutting Input Error");
        }
        private void EmptyBetweenError()
        {
            timerMenu.Stop();
            timerDrawSignal.Stop();
            timerDrawTemp.Stop();
            MessageBox.Show("Please do Not leave empty between hearcutting periods.", "Heartcutting Input Error");
        }
        private void TooCloseError()
        {
            timerMenu.Stop();
            timerDrawSignal.Stop();
            timerDrawTemp.Stop();
            MessageBox.Show("There are two cuttings are too close to each other!", "Heartcutting Input Error");
        }
        ...
        private int heartcuttingflag = 0;
        ...
        private string ExperienceNamevalue;
        ...
        private void timerMenu_Tick(object sender, EventArgs e)//flow control loop
        {
            ...
            if (heartcuttingNumber > 0)
            {
                if (heartcuttingflag == 0) heartcuttingflag = 1;
                if (System.DateTime.Now.Subtract(startTime).TotalMilliseconds < (pumpingTvalue * 1000))
                {
                ...
                {
                    if (System.DateTime.Now.Subtract(startTime).TotalMilliseconds < ((pumpingTvalue + waitingTvalue + heartcuttingStartList[heartcuttingflag]) * 1000))
                    {
                        digitalOutLow(6); preconHighIndicator2.BackColor = Color.Gray;
                        digitalOutLow(7); preconOnIndicator2.BackColor = Color.Gray;
                        digitalOutLow(10); valvaIndicator2.BackColor = Color.Gray;
                    }
                    else if (System.DateTime.Now.Subtract(startTime).TotalMilliseconds < ((pumpingTvalue + waitingTvalue + heartcuttingEndList[heartcuttingflag]) * 1000))
                    {
                        digitalOutHigh(10); valvaIndicator2.BackColor = Color.Green;
                    }
                    else if (System.DateTime.Now.Subtract(startTime).TotalMilliseconds < ((pumpingTvalue + waitingTvalue + heartcuttingEndList[heartcuttingflag] + preconHighTvalue) * 1000))
                    {
                        digitalOutLow(10); valvaIndicator2.BackColor = Color.Gray;
                        digitalOutHigh(6); preconHighIndicator2.BackColor = Color.Green;
                        digitalOutHigh(7); preconOnIndicator2.BackColor = Color.Green;
                    }
                    else if (System.DateTime.Now.Subtract(startTime).TotalMilliseconds < ((pumpingTvalue + waitingTvalue + heartcuttingEndList[heartcuttingflag] + preconHighTvalue + preconLowTvalue) * 1000))
                    {
                        digitalOutLow(10); valvaIndicator2.BackColor = Color.Gray;
                        digitalOutLow(6); preconHighIndicator2.BackColor = Color.Gray;
                        pwmout(50, 7); preconOnIndicator2.BackColor = Color.Yellow;
